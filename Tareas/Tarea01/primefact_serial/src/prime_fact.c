...
int64_t* tamanioArreglo) {
    ...
    for (int64_t i = 3; i <= (int64_t) ceil(sqrt(numero)); i += 2) {
        while (!(numero % i)) {
            arregloFactores = agregarElemento(i, arregloFactores,
            tamanioArreglo);
            numero = numero / i;
        }
    }
    ...
    if (numero > 2) {
        arregloFactores = agregarElemento(numero, arregloFactores,
        tamanioArreglo);
    }
...
int64_t* agregarElemento(int64_t numero, int64_t* arregloFactores,
int64_t* tamanioArreglo) {
...
void impresionDatos(int64_t numero,
